...
(function () {
  ...
  async function loadScheduleServer(params) {
    ...
    const r = await fetchJSON(API_BASE + '/schedules?' + q.toString(), { headers: Object.assign({ 'Accept': 'application/json' }, authHeader()) })
    if (!r.ok) throw new Error(r.body && r.body.error ? r.body.error : ('Server Error ' + r.status))
    return r.body || []
  }

  async function createServerItem(item) {
    const r = await fetchJSON(API_BASE + '/schedules', {
      method: 'POST',
  ...
  async function updateServerItem(id, payload) {
    const r = await fetchJSON(API_BASE + '/schedules/' + encodeURIComponent(id), {
      method: 'PUT',
      headers: Object.assign({ 'Content-Type': 'application/json' }, authHeader()),
